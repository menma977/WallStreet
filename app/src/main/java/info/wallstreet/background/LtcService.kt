      ...
      while (true) {
        ...
        if (delta >= 10000) {
          ...
          if (startBackgroundService) {
            json = GetController("ltc", user.getString("token")).call()
            when {
              json.getInt("code") == 200 -> {
                user.setString("fake_balance_ltc", json.getJSONObject("data").getString("balance"))
